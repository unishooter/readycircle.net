  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <Link
          to="/app/circles/new"
          className="inline-flex items-center justify-center rounded-lg bg-teal-700 px-4 py-2.5 text-sm font-medium text-white hover:bg-teal-800"
        >
          Create a Circle
        </Link>
      </div>
            ...
            <Link key={circle.id} to={`/app/circles/${circle.id}`}>
              <Card className="h-full transition-colors hover:border-teal-300">
                ...
                {circle.viewerRole === 'coordinator' ? (
                  <p className="mt-2 text-xs font-medium text-teal-700">You coordinate this Circle</p>
                ) : null}
              </Card>
            </Link>
          ))}
